...
import { selectCurrentUser, selectEntities, selectIds } from "./profile.reducer";
...
export const getProfileResolver: ResolveFn<any> = (
    ...
    store.pipe(
        select(selectIds)).subscribe(v => ids = v)
    store.pipe(select(selectCurrentUser)).subscribe(v => currentUserId = v?.id)
        ...
        tap((v) => {
            ...
            if (!ids.find((v) => v == currentId)) {
                ...
                if (currentId === 'me') {
                    ...
                    if (myId!==null&&currentUserId !== myId) {
                        ...
                        if (ids.find((v) => v == myId)) {
                            ...
                            store.dispatch(
                              ProfileActions.loadCurrentProfile({ id: myId })
                            );
                        ...
                        store.dispatch(
                                ProfileActions.loadProfile({ id: myId }))
                ...
                return store.dispatch(
                  ProfileActions.loadProfile({ id: currentId })
                );
              }
            } else {


              return of(store.dispatch(
                      ProfileActions.loadCurrentProfile({ id: currentId })
                    ))
                .pipe(
                    concatMap(() => {
                        return store
