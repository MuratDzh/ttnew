...
const adapter: EntityAdapter<Profile> =
...
const ProfileFeatureReducer = createFeature({
  ...
  reducer: createReducer(
    ...
    on(ProfileActions.loadProfileSuccess, (state, action) => {
        return adapter.addOne(action.profile, {...state, isProfileLoaded:true})
    }),
    on(ProfileActions.loadProfileSuccessAfterSubscribeChanged,
      (state, action)=>{
