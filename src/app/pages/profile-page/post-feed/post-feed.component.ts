...
export class PostFeedComponent implements OnDestroy, OnChanges, OnInit {
  ...
  onDelCom(comment: CommentsRes, postAuthorId:number) {
    this.store.dispatch(CommentsActions.deleteComment({comment, postAuthorId}))
  }
  ...
  createCom(text: string, post: PostRes) {
    if (this.isUpCom) {
      ...
      this.store.dispatch(CommentsActions.updateComment({comment:v, postAuthorId:post.author.id}))
      this.isUpCom = false;
      ...
      this.store.dispatch(CommentsActions.createComment({comment, postAuthorId:post.author.id}))
    }
  }
