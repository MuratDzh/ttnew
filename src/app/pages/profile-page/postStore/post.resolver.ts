...
export const getPostResolver: ResolveFn<PostRes[]|string[]|number[]|boolean> = (
    ...
    store.pipe(
        select(selectPostsIds)).subscribe(v => ids = v)
    ...
    return store.pipe(
        ...
        tap((v) => {
            ...
            if (!ids.find((v) => v == currentId)) {
                  ...
                  console.log('IF ME');
                    ...
                    if (!ids.find((v) => v == myId)) {
                        ...
                        return store.dispatch(
                            PostActions.postLoad({ id: myId })
                        )
                    } else {
                        console.log('IF ME YES');
                        return store.select(selectCurrentPostEntities).pipe(
                          map((v) => v[myId]),
                ...
                store.dispatch(
                  PostActions.postLoad({ id: currentId })
                    );
                      console.log('ELSE []');
                      return store.select(selectCurrentPostEntities).pipe(
                        tap((v) => console.log('!!!', v)),
