...
import { selectCurrentPostEntities, selectCurrentPostState } from './currentUserPosts.reducer';
...
export const CurrentUserPostsEffect = createEffect(
  (
    ...
    return actions$.pipe(
      ...
      map((post) => {
        return currentPostActions.getCurrentPostSuccess({ post: post as PostRes });
      })
    );
  },
  { functional: true }
    ...
    return actions$.pipe(
      ...
      switchMap(({ post, id }) => {
        ...
        map((post) => {
            ...
            store.select(selectCurrentPostEntities)
  ...
  (
    ...
    return actions$.pipe(
      ofType(
          ...
          currentPostActions.createPostSuccess
      ),
...
export const UpdateMyPostsAfterDelEffect = createEffect(
    ...
    return actions$.pipe(
      ...
      catchError((errors) =>
        of(currentPostActions.delPostFealure({ errors }))
      )
    );
  },
  { functional: true }
