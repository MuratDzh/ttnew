...
import { CommentInt, CommentsRes, CommentsResFull } from "../../../data/interfces/post.interface.ts";
...
export const CommentsActions = createActionGroup({
  source: 'Profile Page',
  events: {
    'Create Comment': props<{ comment: CommentInt | null , postAuthorId:number|string}>(),
    ...
    'Update Comment': props<{ comment: CommentInt | null , postAuthorId:number|string}>(),
    'Update Comment Success': props<{ comment: Update<CommentsRes | null> }>(),
    'Update Comment Fealure': props<{ errors: BackendErrorsInterface }>(),

    'Delete Comment': props<{ comment:CommentsRes,postAuthorId:number|string }>(),
