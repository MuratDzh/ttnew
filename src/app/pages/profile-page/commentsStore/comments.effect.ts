...
import {CommentInt, CommentsRes, CommentsResFull, PostRes} from "../../../data/interfces/post.interface.ts";
                ...
                map(v=>{
                  console.log("switchMap", v
                    ...
                    return v[comment.author.id]!.posts!.filter(
                      ({id})=>id==comment.postId)[0]
                      .comments.filter(com=>com.id==comment.id)[0]
...
export  const  CreateCommentEffect = createEffect(
  ...
  store=inject(Store))=>{
    return actions$.pipe(
      ...
      tap(({postAuthorId}) => {authorId=postAuthorId}),
  ...
  store=inject(Store))=>{
   ...
   return  actions$.pipe(
      ...
      switchMap(({comment})=>{
        ...
        return firstValueFrom(store.select(selectPostsEntities).pipe(
          map((v)=>{
      ...
      myPosts=myPosts.map(v=>v.id===myComment.postId?v=post:v)
    ...
    return  actions$.pipe(
      ...
      map((comments) => {
        ...
        let post:PostRes={
          ...myPosts!.filter(v=>v.id==myComment.postId)[0],
          comments: [
            ...comments.filter(v=>v.id!==myComment.id)
  ...
  postService=inject(PostService))=>{
    ...
    return actions$.pipe(
      ...
      map((comment)=>{
        ...
        return CommentsActions.updateCommentSuccess({comment:updatedComment})
      })
    )
  }, {functional:true}
)
  ...
  store=inject(Store))=>{
    ...
    return  actions$.pipe(
      ...
      map((comments) => {
        ...
        let comms:CommentsRes[]=[...comments.map(v=>v.id===myComment.id?v=myComment:v)];
        ...
        console.log("ВОЗМОЖНАЯ ОШИБКА", myPosts!.filter(v=>v.id==myComment.post_id)[0])
        ...
        myPosts=myPosts.map(v=>v.id===myComment.post_id?v=post:v)
