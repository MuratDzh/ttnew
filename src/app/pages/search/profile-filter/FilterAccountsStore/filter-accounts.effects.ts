...
import {catchError, filter, first, map, of, Subscription, switchMap} from 'rxjs';
...
export const FilterAccountsEffects = createEffect(
  (actions$ = inject(Actions),
    ...
    store.select(selectSubscriptionsState).pipe(
      filter(v=>!!v),
      ...
      map((accounts) => {
        for(let sub of (subscriptions.items as Profile[])) {
          accounts = {
            ...
            items: [...(accounts.items as Profile[])
              .map(v => v.id ===sub.id? v=sub:v)]
          }
        }
