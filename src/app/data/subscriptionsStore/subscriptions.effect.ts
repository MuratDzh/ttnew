...
import {SubscriptionsActions, UpdateStorsAfterSubscrube} from "./subscriptions.actions";
...
export  const SubscriptionsEffect = createEffect(
  ...
  profileService=inject(ProfileService),)=>{
    return actions$.pipe(
      ...
      map((profiles)=>{
        profiles.items=profiles.items!.map(v=>({...v, isSubscribed: true}))
        return SubscriptionsActions.loadMySubscriptionsSuccess({profiles})

      }),
      catchError(errors=>of(SubscriptionsActions.loadMySubscriptionsFailure({errors})))
    )
  }, {functional:true}
)
...
export const SubscribeEffect=createEffect(
  ...
  profileService=inject(ProfileService)) => {
    ...
    return actions$.pipe(
      ...
      catchError(errors=>of(SubscriptionsActions.subscribeFailure({errors})))
    )
  },{functional:true}
)
   ...
   store=inject(Store))=>{
    ...
    return actions$.pipe(
      ...
      map((v)=>{
        ...
        store.dispatch(ProfileActions.loadCurrentProfileSuccess({profile:newProfile}))
        return ProfileActions.loadProfileSuccessAfterSubscribeChanged({profile: updatedProfile})
      })
    )
  }, {functional:true}
)
   ...
   store=inject(Store))=>{
    ...
    return actions$.pipe(
      ...
      map((accounts)=>{
        ...
        t.items =t.items.map((v:Profile)=>v.id===newProfile.id? v=newProfile : v)
      ...
      //     .map(v=>v.id===newProfile.id? v=newProfile : v)]} as Subscribers<Profile>
        ...
        store.dispatch(FilterAccountsActions.filterAccountsSuccess({accounts:t}))
   ...
   store=inject(Store))=>{
    ...
    return actions$.pipe(
      ...
      map((accounts)=>{
        ...
        store.dispatch(FilterAccountsActions.filterAccountsSuccess({accounts}))
