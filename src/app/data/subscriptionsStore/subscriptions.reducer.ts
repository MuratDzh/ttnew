...
const SubscriptionsFeatureReducer=createFeature({
  ...
  reducer: createReducer(
    ...
    on(SubscriptionsActions.loadMySubscriptionsSuccess, (state, {profiles}) =>{
      return {...state,
    ...
    on(SubscriptionsActions.subscribeSuccess, (state, action) => {
      ...
      return {...state, total: state.total+1, items: [...state.items!, action.profile]};
    }),
    on(SubscriptionsActions.unsubscribeSuccess, (state, action) => {

      return {...state, total: state.total-1, items: [...(state.items as Profile[])?.filter(v=>v.id?v.id!==action.profile.id:v)] };
    }),
  )
})
