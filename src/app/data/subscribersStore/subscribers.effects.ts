...
export const getSubscribersEffects = createEffect((
  ...
  let subscribers: SubEntities
    return actions$.pipe(
        ...
        switchMap(({id}) => {
          if(id==="me"){
            store.select(selectMe).pipe(
              tap(v=>console.log("DDDD", v!.id))
            ).subscribe(
              v=>profileId=(v as Profile).id
            )
          } else {
          profileId=id as number }
          console.log("EFFECT, ID,",id)
          return profileService.getSubscribersById(profileId).pipe(
            tap(v=>console.log("ПРОВЕРКА МЕТОДА", v))
          )
        ...
        catchError((err)=>of(getSubscribersActions.getSubscribersFeilure({errors:err}))
    ))
},{functional:true})
